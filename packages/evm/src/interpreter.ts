...
export class Interpreter {
  ...
  async _runStepHook(dynamicFee: bigint, gasLeft: bigint): Promise<SimpleInterpreterStep> {
   ...
   */
  async call(gasLimit: bigint, address: Address, value: bigint, data: Uint8Array): Promise<BaseCallResult> {
  ...
  async _customSelfDestruct(toAddress: Address): Promise<void> {
    ...
    await this._stateManager.putContractCode(this._env.address, toBuffer('0x'))

    // Add to beneficiary balance
    const toAccount = await this._stateManager.getAccount(toAddress)
    toAccount.balance.iadd(this._env.contract.balance)
    await this._stateManager.putAccount(toAddress, toAccount)

    // Subtract from contract balance
    const account = await this._stateManager.getAccount(this._env.address)
    account.balance = BigInt(0)
    await this._stateManager.putAccount(this._env.address, account)
